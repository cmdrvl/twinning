...
impl BaseSnapshotBackend {
    ...
    pub fn promote_overlay_to_base(&mut self) {
        for (name, table) in std::mem::take(&mut self.overlay_tables) {
            self.base_tables.insert(name, table);
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn promote_overlay_to_base_commits_visible_rest_state() {
        let (deals, sponsors) = deals_and_sponsors_storage();
        let mut backend =
            BaseSnapshotBackend::new([deals.clone(), sponsors]).expect("build backend");
        ...
        assert_eq!(
            backend
                .base_table("public.deals")
                .expect("base deals after promote")
                .row_count(),
            2
        );
    }
