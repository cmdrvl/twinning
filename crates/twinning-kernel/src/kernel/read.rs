...
pub fn execute_scan_all<B: Backend>(
    catalog: &Catalog,
    backend: &B,
    table_name: &str,
    projection: &[String],
    ...
    match execute_scan_all_inner(catalog, backend, table_name, projection, limit) {
        Ok(result) => KernelResult::Read(result),
        Err(refusal) => KernelResult::Refusal(refusal),
    }
}

fn execute_scan_all_inner<B: Backend>(
    catalog: &Catalog,
    backend: &B,
    table_name: &str,
    projection: &[String],
    ...
    let visible = backend.visible_table(table_name).ok_or_else(|| {
    ...
    let rows = visible
        .rows()
        .map(|row| project_row(table.columns.as_slice(), projection, row))
